...
double write_output(char filename[],double *f_x,double *f_y,double num_sys,int num_sys_obs,double binwidth,unsigned int smplpnts) {
  ...
  for(i=0;i<smplpnts;i++) {
    /* x-value center-of-bin (e.g. lP,lE,lL,a,p,e,q,mu); normalized binary fraction per binwidth; corresponds to number of binaries in survey with num_sys_obs targets */
